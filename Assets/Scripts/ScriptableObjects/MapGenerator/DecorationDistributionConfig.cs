...
public class DecorationDistributionConfig : ScriptableObject
{
    ...
    [Range(0, 5)] public int minDenseRegionCount = 0;  // 茂密区域数量 至少
    [Range(0, 5)] public int maxDenseRegionCount = 1;  // 茂密区域数量 至多
    [Range(0, 5)] public int minSparseRegionCount = 0; // 稀疏区域数量 至少
    [Range(0, 5)] public int maxSparseRegionCount = 3; // 稀疏区域数量 至多
    ...
    [Header("向内偏移量")]
    [Range(0, 10)] public float minOffsetLevel1 = 4f;   // 一级装饰物向内偏移量 至少
    [Range(0, 10)] public float maxOffsetLevel1 = 8f;   // 一级装饰物向内偏移量 至多

    [Range(0, 10)] public float minOffsetLevel2 = 4f;   // 二级装饰物向内偏移量 至少
    [Range(0, 10)] public float maxOffsetLevel2 = 10f; // 二级装饰物向内偏移量 至多
}
